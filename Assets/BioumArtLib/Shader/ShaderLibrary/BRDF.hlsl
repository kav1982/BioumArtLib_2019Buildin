...
BRDF GetBRDF(Surface surface, inout half alpha)
{
    ...
    BRDF brdf = (BRDF)0;
    brdf.diffuse = surface.color.rgb * oneMinusReflectivity;

    half3 minReflectivity = MIN_REFLECTIVITY;
#ifndef BIOUM_ADDPASS
    half lum = max(0.001, Luminance(surface.color));
    half3 tint = surface.color.rgb/lum;
    minReflectivity *= LerpWhiteTo(tint, surface.specularTint);
    brdf.fresnel = saturate(surface.smoothness + reflectivity);
#endif

    brdf.specular = lerp(minReflectivity, surface.color.rgb, surface.metallic);
    brdf.perceptualRoughness = 1 - surface.smoothness;
    brdf.roughness = Square(max(0.02, brdf.perceptualRoughness));
...
BRDF GetSimpleBRDF(Surface surface, inout half alpha)
{
    ...
    brdf.diffuse = surface.color.rgb * ONEMINUS_MIN_REFLECTIVITY;
    brdf.perceptualRoughness = 1 - surface.smoothness; 
    brdf.roughness = Square(max(0.05, brdf.perceptualRoughness)); 
    brdf.roughness2 = Square(brdf.roughness);
    ...
    brdf.diffuse *= alpha;
    alpha = alpha * ONEMINUS_MIN_REFLECTIVITY + reflectivity;
#endif
